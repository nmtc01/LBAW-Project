...
|R07                |    vote(__voteID__, like, dislike, userID → user *NN*, questionID → question, answerID → answer *CK questionID = NN XOR answerID = NN*)                       |
|R08                    |question(__questionID__, userID → user *NN*, title *NN*, description *NN*, nrLikes *NN* *DF 0*, nrDislikes *NN* *DF 0*, questionDate *NN DF Today*)|
|R09                    |answer(__answerID__, userID → user *NN*, questionID → question *NN*, answerDate *NN DF Today*, content *NN*, nrLikes *NN* *DF 0*, nrDislikes *NN* *DF 0*)|
|R10                    |comment(__commentID__, userID → user *NN*, questionID→Question, answerID→Answer *CK questionID = NN XOR answerID = NN*, content *NN*, commentDate *NN DF Today*)|
|R11                    |report(__reportID__, userID→User, questionID→Question, answerID→Answer, commentID→Comment *CK userID = NN XOR questionID = NN XOR answerID = NN XOR commentID = NN*)| 
|R12                    |report_status(__statusID__, reportID→Report, state *NN DF unresolved CK state IN States*, comment, responsibleUser→Moderator *NN*)|
|R13                    |marked_answer(__questionID__ → question, __answerID__ → answer)|
